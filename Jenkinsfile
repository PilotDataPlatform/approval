pipeline {
    agent { label 'small' }
    environment {
      imagename_dev = "ghcr.io/pilotdataplatform/approval"
      imagename_staging = "ghcr.io/pilotdataplatform/approval"
    ...
    stage('Git clone for dev') {
        ...
        steps{
          script {
          git branch: "develop",
              url: 'https://github.com/PilotDataPlatform/approval',
              credentialsId: 'lzhao'
            }
        }
    }
    ...
    stage('DEV Deploy') {
      ...
      steps{
        build(job: "/VRE-IaC/UpdateAppVersion", parameters: [
          ...
          [$class: 'StringParameterValue', name: 'TARGET_RELEASE', value: 'approval' ],
